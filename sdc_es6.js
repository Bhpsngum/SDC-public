...
let introductory_paragraph = ["Welcome to Starblast Dueling Championship (SDC)!", "Choose your favorite ship and press ready to participate in 1v1 duels and become the champion!", "Also, please note that you cannot shoot spectators, and they cannot hurt you!", "Good luck and have fun!"];
...
if (!game.custom.ship_infos) {
  let ship_infos = ships.map(i => {
    ...
    return {
      ...
      max_ship_items = Math.max(1, max_item_per_ship_selection_screen) || 1,
      temp = [];
...
let getShipsPage = function (ship) {
  let custom = (ship === null || ship === void 0 ? void 0 : ship.custom) || {},
      ship_info = ship_infos[custom.ships_page];
  if (Array.isArray(ship_info)) return {
    index: +custom.ships_page,
    list: ship_info
  };
  custom.ships_page = 0;
  return {
...
let map_size = 200,
  ...
  return Math.floor(Math.random() * num);
    ...
    n = Math.round(Math.max(nodes_count_per_width, 1)),
    vocabulary = [{
  text: "Duel",
  icon: "\u00be",
  key: "D"
...
let modUtils = {
  ...
  return [{
    ...
    stroke: fill,
    fill: `hsla(${h}, ${s}%, ${l}%, ${button_opacity})`,
    width: 10 * scale
  }, {
    type: 'round',
    position: [-radius, -radius, diameter, diameter],
    fill: fill
  }, {
    type: 'round',
    position: [100 - radius, -radius, diameter, diameter],
    fill: fill
  }, {
    type: 'round',
    position: [100 - radius, 100 - radius, diameter, diameter],
    fill: fill
  }, {
    type: 'round',
    position: [-radius, 100 - radius, diameter, diameter],
    fill: fill
  }, ...custom];
},
  ...
  sendUI(ship, {
    ...
    position,
    visible,
    clickable: clickable !== null && clickable !== void 0 ? clickable : visible,
    shortcut,
  ...
  let t = 0,
      w = 50 / n,
      nearest = ArenaManager.findNearest(ship);
  ArenaManager.list.forEach(arena => {
    let text = arena.lobby ? "Lobby" : "Arena " + arena.index,
        i = arena.originalIndex;
    let compo = [{
      type: "text",
      position: [20, 0, 60, 100],
      value: text
    }];
    arena === nearest && compo.push({
      type: "text",
      position: [20, 0, 60, 40],
      value: "You are here"
    });
    !arena.isAvailable && !arena.lobby && compo.push({
      type: "text",
      position: [20, 60, 60, 40],
      value: "Duel ongoing"
    });
  ...
  let ship_info = getShipsPage(ship).list,
      wt = Math.PI * 2 / ship_info.length,
      br = 21.5;
  ship_info.forEach((info, i) => {
    let nspace = new Array(maxNamelength - info.name.length).fill(" ").join(""),
        dspace = new Array(maxDesignerlength - info.designer.length).fill(" ").join(""),
        compo = [{
      type: "text",
      position: [10, 0, 80, 100],
      value: nspace + info.name + nspace
    }];
    ship.type === info.code && compo.push({
      type: "text",
      position: [10, 0, 80, 40],
      value: "    ✓    "
    });
    info.designer && compo.push({
      type: "text",
      position: [10, 60, 80, 40],
      value: dspace + "by: " + info.designer + dspace
    });
  ...
  sendUI(ship, {
    ...
    components: [{
      type: "text",
      position: [0, 65, 100, 10],
      value: "Press 7 for previous and 8 for next ship",
      color: dfl_tcl
    }, {
      type: "round",
      position: [0, 0, 100, 30 / 60 * 100],
      fill: "hsla(0, 0%, 0%, 0)",
      width: 1,
      stroke: dfl_tcl
    }, ...(multipleShipsPages ? [{
      type: "text",
      position: [0, 85, 100, 10],
      value: `Page ${ship.custom.ships_page + 1}/${ship_infos.length}`,
      color: dfl_tcl
    }] : [])]
  });
  ...
  sendUI(ship, {
    ...
    components: [{
      type: "text",
      position: [0, 0, 100, 100],
      value: "Press 9 or 0 to toggle between arenas",
      color: dfl_tcl
    }]
  });
  ...
  for (let id = 0; id <= game.custom.maxID; ++id) {
    ...
    if (ship.custom.invite_shown && isActive && !bool && (s = game.findShip(id)) != null && s != ship && !s.custom.isReady() && !s.custom.inMatch()) {
      ...
      ++index;
    } else for (let jid of ["invite", "accept", "decline"]) sendUI(ship, {
      id: jid + id,
      visible: false
    });
  }
  ...
  let ui_list = [];
  if (ship.custom.invite_page == total_invite_pages) ship.custom.invite_page = 0;else if (ship.custom.invite_page < 0) ship.custom.invite_page = total_invite_pages - 1;
  ships_list.forEach(({
    id,
    UIList
  }, index) => {
    if (ship.custom.invite_page * total_invites_per_page <= index && index < (ship.custom.invite_page + 1) * total_invites_per_page) {
      ...
      let s = game.findShip(id);
      let row = Math.trunc(index / invite_columns),
          column = index % invite_columns;
      let offsetX = InviteScreen.offsetX + (column + 1) * actual_width,
          offsetY = InviteScreen.offsetY + (row * pHeight + 10) * InviteScreen.height / 100;
      ...
      ui_list.push({
        ...
        fill: `hsla(${infoHue}, 100%, 50%, ${button_opacity})`,
        forceCustomFill: true
      }, {
        type: "player",
        id: id,
        position: [actual_position[0], actual_position[1] + (pHeight - playerTextHeight) / 2, actual_position[2], playerTextHeight],
        color: `hsla(${infoHue}, 100%, 50%, 1)`,
        forceCustomColor: true,
        align: "left"
      });
    } else for (let jid of ["invite", "accept", "decline"]) sendUI(ship, {
      id: jid + id,
  ...
  buttonCustomUI(ship, "invite_screen", InviteScreen.position, !!ship.custom.invite_shown && isActive && !bool, false, null, [{
    type: "text",
    position: [InviteScreen.margin.left, 0, InviteScreen.actualWidthPercentage, InviteScreen.margin.top],
    value: "INVITES"
  }, ...(multipleInvitePages ? [{
    type: "text",
    position: [InviteScreen.margin.left, 100 + ((10 - textHeight + 1) / 20 - 1) * InviteScreen.margin.bottom, InviteScreen.actualWidthPercentage, InviteScreen.margin.bottom * (textHeight - 1) / 10],
    value: `Page ${ship.custom.invite_page + 1}/${total_invite_pages}`,
    color: dfl_tcl
  }] : []), {
    type: "text",
    position: [0, 0, pWidth - pHeight * UIScale * 2, playerTextHeight],
    value: " "
  }, // player component scale
  ...ui_list], 60, 100, 50, 0.25);
},
    InviteButtons = [{
  name: "invite",
  symbol: "✉",
...
r = arena_radius * 10,
  ...
  return Math.max(o * zoom - rsize, 0) || 0;
    ...
    grids = Array(n).fill(0).map((f, i) => Array(n).fill(0).map((v, j) => [-map_size * 5 + pos(j), map_size * 5 - pos(i)])).flat(),
    lobby = Math.trunc((grids.length - 1) / 2),
    leaderboard = [],
    getStat = function (ship, stateName) {
  ...
  leaderboard = game.ships.map(ship => ({
    ...
    let d01 = Math.max(a.wins, a.losses, a.draws),
        d02 = Math.max(b.wins, b.losses, b.draws);
  ...
  leaderboard.forEach((data, index) => {
    ...
    if (ship != null) {
      ...
      ship.set({
        score: leaderboard.length - ship.custom.rank
      });
    }
  });
  let t = ["Wins", "Draws", "Losses"],
      c = [120, 60, 0],
      tcl = [black, black, dfl_tcl],
      f = leaderboard.slice(0, 10),
      indents = t.map(i => (Math.max(...f.map(x => x[i.toLowerCase()])) || 0).toString().length * textHeight / 2 + textHeight / 2),
      idn = indents.map((i, j) => 100 - indents.slice(j, 3).reduce((a, b) => a + b, 0)),
      scoreboard = {
    id: "scoreboard",
    visible: true,
    position: [0, 0, 100, 100],
    components: [{
      type: "box",
      position: [0, 0, 100, 100 / 11],
      fill: "hsla(210, 20%, 33%, 1)"
    }, {
      type: "text",
      position: [0, offsetY, 100, textHeight],
      color: lcolor,
      value: "Players",
      align: "left"
    }, {
    ...
    ...t.map((field, i) => [{
      type: "box",
      position: [idn[i], 0, indents[i], 100 / 11],
      fill: "hsla(" + c[i] + ", 100%, 50%, 1)"
    }, {
      type: "text",
      position: [idn[i], offsetY, indents[i], textHeight],
      color: tcl[i],
      value: field[0]
    }]).flat(), ...f.map((info, i) => {
      var _game$findShip, _game$findShip$custom, _game$findShip$custom2;

      return [{
        type: "player",
        position: [0, (i + 1) * 100 / 11, idn[0], 100 / 11],
        id: info.id,
        color: (_game$findShip = game.findShip(info.id)) !== null && _game$findShip !== void 0 && (_game$findShip$custom = _game$findShip.custom) !== null && _game$findShip$custom !== void 0 && (_game$findShip$custom2 = _game$findShip$custom.isReady) !== null && _game$findShip$custom2 !== void 0 && _game$findShip$custom2.call(_game$findShip$custom) ? "hsla(120,100%,50%,1)" : lcolor,
        align: "left"
      }, ...t.map((stat, j) => [{
        type: "text",
        position: [idn[j], (i + 1) * 100 / 11 + offsetY, indents[j], textHeight],
        color: lcolor,
        value: getStat(game.findShip(info.id), stat.toLowerCase()),
        align: "right"
      }]).flat()];
    }).flat()]
  };
  ...
  for (let ship of game.ships) {
    let csc = JSON.parse(JSON.stringify(scoreboard)),
        index = leaderboard.slice(0, 10).map(i => i.id).indexOf(ship.id);

    if (index == -1) {
      index = 9;
      csc.components.splice(csc.components.length - 4, 4, {
        type: "player",
        position: [0, 1000 / 11, 100, 100 / 11],
        id: ship.id,
        color: lcolor,
        align: "left"
      }, ...t.map((stat, j) => [{
        type: "text",
        position: [idn[j], 1000 / 11 + offsetY, indents[j], textHeight],
        color: dfl_tcl,
        value: getStat(ship, stat.toLowerCase()),
        align: "right"
      }]).flat());
    }

    csc.components.unshift({
      type: "box",
      position: [0, (index + 1) * 100 / 11, 100, 100 / 11],
      fill: "hsla(210, 24%, 29%, 0.5)"
    });
    sendUI(ship, csc);
  }
},
  ...
  if (ship != null) {
    if (ship.custom.spectate) ship.set({
      type: spectator
    });else {
    ...
    if (!ship.custom.joined) {
      ...
      sendUI(ship, {
        ...
        components: [{
          ...
          type: "text",
          position: [0, 0, 100, 100],
          value: "We don't use points here.",
          color: black
        }]
      });
      sendUI(ship, {
      ...
      clearInds(ship);
      introductory_paragraph.forEach((sentence, i) => modUtils.setTimeout(function () {
        ship.instructorSays(sentence, "Zoltar");
      }, i * instructor_duration * 60));
      ...
      ship.custom = {
        ...
        isReady: function () {
          return (this.ready || this.paired) && !this.spectate && this.ship.alive && this.arena.lobby;
        },
        inMatch: function () {
          return !this.arena.lobby && (this.arena.inMatch || this.arena.inCountdown);
        },
        ships_page: 0,
        inviters: new Map(),
        inviting: new Map(),
  ...
  if (game.step % 30 === 0 || forced) {
    for (let ship of game.ships) {
      let t = ship.custom.arena,
          tp = ship.custom.TpTimestamp,
          ptp = ship.custom.pendingTp;
      ...
      let grad = t.radius,
          distance = t.distTo(ship) - (t.lobby ? r : grad),
          text = "Warning: Out of the safe zones!";

      if (distance > 0 && (tp == -1 || t.lobby) && ptp == -1 && !ship.custom.spectate) {
        if (t.lobby) ArenaManager.set(ship, lobby, true);else {
      ...
      while (i < tw.length) {
        let wt = tw[i];
        if (typeof wt[1] == "number" && wt[1] !== -1 && game.step - wt[1] > messageHoist * 60) tw.splice(i, 1);else i++;
      }
      ...
      ship.custom.pendingTp = ptp; // text announced

      let arenaStatus = ship.custom.arena,
          announceText = [];
      if (!ArenaManager.isRunning(game)) announceText = ['Game finished!', '', ship.custom.endgameText || ""];else if (arenaStatus.lobby) {
        if (!ship.custom.invite_shown) {
          announceText = ["", [("You are " + (ship.custom.isReady() ? "" : "not ") + "ready").toUpperCase(), ship.custom.isReady() ? 120 : 10]];
          let header = ["Welcome to SDC 2.0"],
              nearest = ArenaManager.findNearest(ship);
          ...
          if (ship.custom.spectate && nearest !== ArenaManager.lobby) {
            ...
            if (!nearest.isAvailable) {
              if (nearest.inCountdown) header.push('Dueling in countdown: ' + FormatTime(nearest.countdown));else if (nearest.inMatch) header.push('Duel ends in: ' + FormatTime(nearest.duration));else header.push('Duel finished');
      ...
      announce.call(this, ship, ...announceText); // small text

      let smallText = [];
      if (arenaStatus.lobby) smallText = ["Duelers in queue: " + ArenaManager.waiting_count + (ArenaManager.waiting_time > 0 ? ` (${Math.trunc(ArenaManager.waiting_time / 60)})` : "")];else if (arenaStatus.inCountdown) smallText = ["Preparing ships..."];else smallText = ["Time left: " + FormatTime(arenaStatus.duration)];
  ...
  if (allow_full_cargo_pickup) for (let ship of game.ships) {
    if (ship.custom.inMatch()) {
      ...
      if (ship.crystals == maxgems) ship.set({
        crystals: maxgems - 1
      });
    }
  }
  ...
  if (ship.custom.arena.lobby || forced) {
    let w = "";
    let verbs = ["won", "drew", "lost"],
        pstats = ["Win", "Draw", "Lose"];
    w = "You ";
  ...
  sendUI(ship, {
    ...
    visible: true,
    clickable: false,
    components: war.map(message => ({
      type: "text",
      position: [0, message[2] != null ? message[2] : wh * t++, 100, wh],
      value: message[0],
  ...
  sendUI(ship, {
    ...
    position: [3, 28, 17, 15],
    visible: true,
    components: stats.flat().map((j, i) => ({
      type: "text",
      position: [0, 33 * i, 80, 33],
      value: j,
      color: dfl_tcl
    }))
  });
  ...
  update: function (game, needsUpdate) {
    this.UI.components = ArenaManager.list.map(p => {
      ...
      return {
        type: "round",
        position: [t(p.x, rsize), t(-p.y, rsize), rsize * 2, rsize * 2],
        width: 1,
        stroke: "hsla(240,100%,50%,1)",
        fill: "hsla(0, 0%, 0%, 0)"
      };
    });
  ...
  set: function (ship) {
    ...
    parseUI = function (UI) {
  try {
    UI = new Object(JSON.parse(JSON.stringify(UI)));
  } catch (e) {
    UI = {};
  }
  ...
  if (parsedUI.visible || parsedUI.visible == null) {
    ...
    let position = parsedUI.position,
        count = 0;
...
let Arena = class Arena {
  constructor(x, y, index) {
    _defineProperty(this, "getRadius", function () {
      let percentage = (shrink_start_time - this.duration / 3660) / (shrink_start_time - shrink_end_time) || 1,
          rad = min_radius_ratio === false ? arena_radius - fixed_min_radius : arena_radius * min_radius_ratio || 0;
    ...
    _defineProperty(this, "dc_time", toTick(duel_countdown / 60));

    _defineProperty(this, "game_time", toTick(duel_duration));
  ...
  assign(ship1, ship2) {
    ...
    this.members.forEach((ship, nodeInd) => {
      ...
      ship.set({
        idle: true,
        type: ship.custom.type + ships_count,
        stats: 88888888,
        vx: 0,
        vy: 0
      });
  ...
  shrink(game, ignoreRadarUpdate = false) {
    let prad = this.radius,
        grad = this.getRadius() * 10;

    if (prad != grad) {
      this.radius = grad;
      let shrinkSize = arena_radius - grad / 10;
      let objrad = 23.5 * (arena_radius - shrinkSize),
          ar = 23.5 * arena_radius;
...
if (!game.custom.ArenaManager) game.custom.ArenaManager = {
  findAvailable: function () {
    return this.list.find(v => v.isAvailable);
  },
  waiting_count: 0,
  list: [],
  invites: [],
  ...
  checkPlayers: function (game) {
    ...
    if (this.isOpen(game)) {
      ...
      if (this.findAvailable() && waiting_list.length > 1) {
        if (this.waiting_time < 0) this.waiting_time = queueWaitingTime * 60 + 45;else --this.waiting_time;
  ...
  isOpen: function (game) {
    return game.step <= toTick(this.game_duration);
  },
  isRunning: function (game) {
    return this.isOpen(game) || this.list.filter(arena => arena.lobby || arena.isAvailable).length != this.list.length;
  },
  set: function (ship, node, forced, reversed) {
    ...
    if (ship != null) {
      ...
      if (ship.custom.pendingTp === -1 || forced) {
        ...
        let angle = 0,
            distance = 0,
            noded = !nodpos.lobby && !ship.custom.spectate;
  ...
  findNearest: function (ship) {
    let t = this.list.map(i => i.distTo(ship)),
        min = Math.min(...t);
...
let initialization = function (game) {
  ...
  check(game);
  if (ArenaManager.isRunning(game)) ArenaManager.checkPlayers(game);else {
    game.setOpen(false);
    check(game, true);
    updatescoreboard(game);
    ...
    leaderboard.forEach((i, j) => {
      ...
      if (ship != null) {
        let text = "";
        if (lastStand || Math.max(i.wins, i.losses, i.draws) == 0) lastStand = true;else {
          let rank = ship.custom.rank;

          if (!isNaN(rank)) {
            text = "You";
            if (rank == 1) text += " win";else {
              text += "'re ranked " + rank;

              switch (rank % 10) {
                case 1:
                  if (Math.trunc(rank / 10) != 1) {
                    text += "st";
                    break;
                  }
...
this.event = function (event, game) {
  ...
  if (ship != null) switch (event.name) {
            ...
            default:
              switch (true) {
                ...
                case id.match(/^s\d+$/) && !ship.custom.spectate && ship.custom.shipped:
                  {
                    let shipCode = +id.slice(1),
                        page = getShipsPage(ship).list;
                ...
                case id.match(/^invite\d+$/) && !ship.custom.isReady():
                  {
                    ...
                    if (!Iship || Iship === ship) addWarn(ship, "Player not found!");else if (Iship.custom.paired || Iship.custom.isReady() || ship.custom.inMatch()) addWarn(ship, "Player is not available right now.");else {
                      let invite_waiting = Math.ceil((InvitationTimeOut * 60 - (game.step - ship.custom.invite_timeout.get(Iship.id))) / 60);
                      if (invite_waiting > 0) addWarn(ship, `Please wait for ${invite_waiting} second${invite_waiting != 1 ? "s" : ""} before you can invite ${Iship.name} again`);else if (!ship.custom.inviters.get(Iship.id)) {
                ...
                case id.match(/^accept\d+$/) && !ship.custom.isReady():
                  {
                    let Iship = game.findShip(+id.match(/\d+/)[0]);
                    if (!Iship || Iship === ship) addWarn(ship, "Player not found!");else if (Iship.custom.paired || Iship.custom.isReady() || ship.custom.inMatch()) addWarn(ship, "Player is not available right now.");else if (ship.custom.inviters.get(Iship.id)) {
                ...
                case id.match(/^decline\d+$/) && !ship.custom.isReady():
                  {
                    let Iship = game.findShip(+id.match(/\d+/)[0]);
                    if (!Iship || Iship === ship) addWarn(ship, "Player not found!");else {
                      let isInviter = !!ship.custom.inviters.get(Iship.id),
                          isInviting = !!ship.custom.inviting.get(Iship.id);
    ...
    case "ship_destroyed":
      if (!ship.custom.arena.lobby && ship.custom.arena.started) ship.custom.arena.endDuel(false, ship);else resetShip(ship);
      break;
